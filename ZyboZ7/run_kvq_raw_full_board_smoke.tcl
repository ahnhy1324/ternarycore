...
proc run_board_smoke {build_root evidence_root expected_serial} {
    ...
    puts "KVQ_RAW_FULL_BOARD_SMOKE_PASS context=[dict get $data context] axi_width=$AXI_DATA_WIDTH scale_width=$scale_width profile=[dict get $identity_info profile] qk_style=[dict get $identity_info qk_style] av_style=[dict get $identity_info av_style] k_ddr_base=[hex32 $K_DDR_BASE] v_ddr_base=[hex32 $V_DDR_BASE] first_generation=[dict get $first generation] restart_generation=[dict get $restart generation] scores=[expr {4 * [dict get $data context]}] numerators=512 normalized=512 clear_hidden=pass abort_hidden=pass fault_hidden=pass no_reset_restart=pass"
}
...
if {$argc == 1 && [lindex $argv 0] eq "--self-test"} {
    ...
    puts stderr "usage: xsdb ZyboZ7/run_kvq_raw_full_board_smoke.tcl <build-root> <wrapper-evidence-root> <expected-cable-serial>"
    puts stderr "       xsdb ZyboZ7/run_kvq_raw_full_board_smoke.tcl --self-test ?12|16?"
    exit 2
}
