# Portable Vivado 2025.1 IP packaging for the raw-V5 AV diagnostic.
...
# Zynq-7000 Production core, and rediscovers the core in an xc7z020 project.
#
...
#   vivado -mode batch \
#     -log D:/tc-logs/axi-v5-av-diag-package.log \
#     -journal D:/tc-logs/axi-v5-av-diag-package.jou \
#     -source ip/package_axi_v5_av_diag_portable.tcl \
#     -tclargs REPO_ROOT OUTPUT_ROOT PART CLOCK_HZ M_AXI_DATA_WIDTH
...
if {$argc != 5} {
    error "usage: package_axi_v5_av_diag_portable.tcl REPO_ROOT OUTPUT_ROOT PART CLOCK_HZ M_AXI_DATA_WIDTH"
}
...
if {![string is integer -strict $m_axi_data_width] ||
    ...
    error "M_AXI_DATA_WIDTH must be exactly 64 or 128; got '$m_axi_data_width'"
}
...
set ip_name "axi_v5_av_diag"
set ip_vendor "shepherdscientific.com"
set ip_library "user"
set ip_version "1.0"
set ip_vlnv "${ip_vendor}:${ip_library}:${ip_name}:${ip_version}"
set ip_display "Raw V5 AV Numerator Diagnostic"
set ip_desc "Checkpoint-independent dense signed-V5 AXI reader with local exponent/UQ4.8 metadata and readable signed 48-bit AV numerators."
...
append identity "flow=axi_v5_av_diag_portable_package\n"
...
set user_m_axi_width [ipx::get_user_parameters M_AXI_DATA_WIDTH \
    -of_objects $core]
set hdl_m_axi_width [ipx::get_hdl_parameters M_AXI_DATA_WIDTH \
    -of_objects $core]
set_property value $m_axi_data_width [lindex $user_m_axi_width 0]
set_property value $m_axi_data_width [lindex $hdl_m_axi_width 0]

# Both AXI interfaces share the one board fabric clock and active-low reset.
foreach busif_name {s_axi m_axi clk rst_n} {
...
puts "PORTABLE_IP_PACKAGE_PASS vlnv=$ip_vlnv part=$part clock_hz=$clock_hz m_axi_data_width=$m_axi_data_width root=$package_root"
