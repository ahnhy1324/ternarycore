# Portable Vivado 2025.1 IP packaging for the raw K4/QK/softmax/V5/AV diagnostic.
...
# Zynq-7000 Production core with exact 64/128-bit master provenance, and
...
#   vivado -mode batch \
#     -log D:/tc-logs/axi-kvq-raw-full-package.log \
#     -journal D:/tc-logs/axi-kvq-raw-full-package.jou \
#     -source ip/package_axi_kvq_raw_full_diag_portable.tcl \
#     -tclargs REPO_ROOT OUTPUT_ROOT PART CLOCK_HZ M_AXI_DATA_WIDTH SCALE_WIDTH QK_MULT_STYLE AV_MULT_STYLE
...
if {$argc != 8} {
    error "usage: package_axi_kvq_raw_full_diag_portable.tcl REPO_ROOT OUTPUT_ROOT PART CLOCK_HZ M_AXI_DATA_WIDTH SCALE_WIDTH QK_MULT_STYLE AV_MULT_STYLE"
}
...
set m_axi_data_width [string trim [lindex $argv 4]]
set scale_width [string trim [lindex $argv 5]]
...
if {![string is integer -strict $m_axi_data_width] ||
    ...
    error "M_AXI_DATA_WIDTH must be exactly 64 or 128 for the Zybo raw-full gate; got '$m_axi_data_width'"
}
if {![string is integer -strict $scale_width] ||
    $scale_width ni {12 16}} {
    error "SCALE_WIDTH must be exactly 12 or 16; got '$scale_width'"
}
if {![string is integer -strict $qk_mult_style] || $qk_mult_style != 2} {
    error "QK_MULT_STYLE must be exactly 2 (AUTO) for the Zybo raw-full profiles; got '$qk_mult_style'"
}
...
set ip_name "axi_kvq_raw_full_diag"
set ip_vendor "shepherdscientific.com"
set ip_library "user"
set ip_version "1.0"
set ip_vlnv "${ip_vendor}:${ip_library}:${ip_name}:${ip_version}"
set ip_display "Raw K4/QK/Softmax/V5/AV Diagnostic"
set ip_desc "Checkpoint-independent raw K4 QK, guarded score-row commit, softmax, raw V5 read, and AV diagnostic with independent configurable-width K/V AXI masters."
set supported_families {zynq Production}
set catalog_smoke_parts [list $part]

set rtl_files {}
foreach rtl_name {
    axi_kvq_raw_full_diag.v
    kv_v03_raw_k4_qk_engine.v
    kv_v03_hp64_range_reader.v
    qk_group_dot.v
    kv_v03_qk_score_quantizer.v
    kv_v03_score_row_commit_guard.v
    kv_v03_softmax_av_pipeline.v
    kv_v03_softmax_engine.v
    kv_v03_softmax.v
    kv_v03_score_store.v
    kv_v03_exp_lut.v
    kv_v03_reciprocal.v
    kv_v03_raw_v5_av_engine.v
    kv_v03_raw_v5_axi_reader.v
...
append identity "flow=axi_kvq_raw_full_diag_portable_package\n"
...
append identity "m_axi_data_width=$m_axi_data_width\n"
append identity "scale_width=$scale_width\n"
append identity "qk_mult_style=$qk_mult_style\n"
append identity "av_mult_style=$av_mult_style\n"
append identity "raw_full_profile=$raw_full_profile\n"
...
# All three AXI interfaces share the one board fabric clock and active-low reset.
foreach busif_name {s_axi m_axi_k m_axi_v clk rst_n} {
...
ipx::associate_bus_interfaces -busif m_axi_k -clock clk $core
ipx::associate_bus_interfaces -busif m_axi_v -clock clk $core
...
foreach required_busif {s_axi m_axi_k m_axi_v} {
...
append catalog_result "m_axi_data_width=$m_axi_data_width\n"
append catalog_result "scale_width=$scale_width\n"
append catalog_result "qk_mult_style=$qk_mult_style\n"
append catalog_result "av_mult_style=$av_mult_style\n"
append catalog_result "raw_full_profile=$raw_full_profile\n"
...
puts "PORTABLE_IP_PACKAGE_PASS vlnv=$ip_vlnv part=$part clock_hz=$clock_hz m_axi_data_width=$m_axi_data_width scale_width=$scale_width qk_mult_style=$qk_mult_style av_mult_style=$av_mult_style raw_full_profile=$raw_full_profile root=$package_root"
