...
// The canned diagnostic fixes MAX_CONTEXT at 128.  A flat, single-read-port
// score bank maps cleanly to one RAMB resource on XC7Z020; the generic 2-D
// guard remains available to the other integration paths unchanged.
module kv_v03_canned_score_row_guard #(
    parameter integer MAX_CONTEXT = 128,
    ...
    (* ram_style = "block" *) reg signed [15:0] score_mem [0:511];
                ...
                case (state)
                    ...
                    ST_LOAD: begin
                        if (score_fire) begin
                            if (!score_tag_ok) begin
                                ...
                                score_mem[{score_row, score_index[6:0]}] <=
                                    score_data;
                    ...
                    ST_ACTIVE: begin
                        if (bank_release) begin
                            ...
                            rd_data <= score_mem[{rd_row, rd_index[6:0]}];
                            rd_valid <= 1'b1;
                        end
                    end
...
`ifndef SYNTHESIS
    initial begin
        if (MAX_CONTEXT != 128)
            $error("kv_v03_canned_score_row_guard requires MAX_CONTEXT=128");
        if (TAG_WIDTH < 1)
            $error("kv_v03_canned_score_row_guard TAG_WIDTH must be positive");
    end
`endif
endmodule
